...
def analytics_html(metadata):
    ...
    template = """<script>
  window.sa_metadata = __METADATA__;
  ...
  (function () {
    ...
    return template.replace("__METADATA__", metadata_json)
...
<header class="header">
  ...
  <nav><a href="/">Vandaag</a><a href="/edities/">Archief</a><a href="/over/">Over</a><a class="inbox" href="/#nieuwsbrief">In je inbox</a></nav>
</header>
...
def render_archive(entries):
    ...
    for entry in entries:
        ...
        rows.append(f"""<a class="archive-item" href="/edities/{date_value}/">
      <span class="archive-date">{esc(short_date)}</span>
      <span class="archive-title">Positief nieuws · {esc(date_text)}</span>
      <span class="archive-arrow">→</span>
    </a>""")

    archive_rows = "\n".join(rows)
    canonical = f"{SITE_URL}/edities/"
    desc = "Bekijk alle eerdere edities van Positief nieuws: per editie 12 positieve nieuwsverhalen uit Nederland en de wereld, plus 3 belangrijke onderwerpen."

    schema = json.dumps({
        "@context": "https://schema.org",
        "@type": "CollectionPage",
        "name": "Archief van Positief nieuws",
        "url": canonical,
        "description": desc,
        "isPartOf": {
            "@type": "WebSite",
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Archief | Eerdere edities van Positief nieuws</title>
  <meta name="description" content="{esc(desc)}">
  <link rel="canonical" href="{canonical}">
  <meta property="og:title" content="Archief · Positief nieuws">
  <meta property="og:description" content="{esc(desc)}">
  ...
  <style>
    :root{{--paper:#f7f5ec;--ink:#151a17;--green:#1f5b45;--green-dark:#173f31;--accent:#d6a13a;--muted:#68716b;--line:rgba(23,63,49,.22);--line2:rgba(23,63,49,.12);--content:748px;--header:1080px}}
    *{{box-sizing:border-box}}html{{font-size:17px}}body{{margin:0;background:var(--paper);color:var(--ink);font-family:Arial,Helvetica,sans-serif;line-height:1.58;-webkit-font-smoothing:antialiased}}
    a{{color:inherit}}.header{{width:min(calc(100% - 48px),var(--header));margin:auto;padding:26px 0 18px;display:flex;align-items:center;justify-content:space-between;gap:24px}}
    .brand{{display:inline-flex;align-items:center;gap:12px;font-weight:800;text-decoration:none;letter-spacing:-.02em}}.sun{{width:28px;height:28px;color:var(--accent)}}nav{{display:flex;align-items:center;gap:22px}}nav a{{font-size:.8rem;font-weight:650;text-decoration:none}}.inbox{{padding:8px 14px;border:1px solid rgba(214,161,58,.45);border-radius:999px;color:var(--accent)}}
    .shell{{width:min(calc(100% - 48px),var(--content));margin:auto}}.hero{{padding:72px 0 38px}}.kicker{{margin:0 0 8px;color:var(--green);font-size:.68rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase}}h1,.archive-title{{font-family:Georgia,"Times New Roman",serif}}h1{{margin:0;font-size:clamp(2.8rem,6vw,4rem);line-height:1;letter-spacing:-.05em}}h1 b{{color:var(--accent)}}.lead{{max-width:650px;margin:20px 0 0;color:#313934;font-size:1rem}}.rule{{margin-top:30px;border-top:1px solid var(--line)}}
    .content{{padding:24px 0 76px}}.archive-list{{margin-top:26px;border-top:1px solid var(--green-dark)}}.archive-item{{display:grid;grid-template-columns:150px minmax(0,1fr) 28px;gap:18px;align-items:center;padding:22px 0;border-bottom:1px solid var(--line2);text-decoration:none}}.archive-date{{color:var(--green);font-size:.72rem;font-weight:750}}.archive-title{{font-size:1.15rem;line-height:1.2}}.archive-arrow{{text-align:right;color:var(--green)}}.archive-item:hover .archive-title{{text-decoration:underline;text-underline-offset:3px}}.small{{color:var(--muted);font-size:.82rem;margin-top:25px}}footer{{padding:0 0 34px;text-align:center;color:#8b928e;font-size:.72rem}}
    @media(max-width:760px){{.header,.shell{{width:min(calc(100% - 30px),100%)}}.header{{padding:18px 0 10px;align-items:flex-start}}nav{{gap:10px}}nav a{{font-size:.65rem}}.hero{{padding-top:52px}}.archive-item{{grid-template-columns:92px minmax(0,1fr) 20px;gap:10px}}.archive-title{{font-size:1rem}}}}
  </style>
</head>
<body>
<header class="header">
...
<main>
  <section class="hero"><div class="shell">
    <p class="kicker">Eerdere edities</p>
    <h1>Archief<b>.</b></h1>
    <p class="lead">Lees eerdere edities van Positief nieuws terug. Iedere editie bevat zes positieve verhalen uit Nederland, zes uit de wereld en drie belangrijke onderwerpen om bij te blijven.</p>
    <div class="rule"></div>
  </div></section>
  <section class="content"><div class="shell">
    <div class="archive-list">{archive_rows}</div>
    <p class="small">De nieuwste editie vind je altijd op de <a href="/">homepage</a>.</p>
  </div></section>
</main>
<footer>Positief nieuws · Dit gebeurt ook.</footer>
{analytics}
...
def main():
    ...
    (EDITIONS_DIR / "index.html").write_text(
    ...
    sitemap_urls = [
        ...
        f"  <url>\n    <loc>{SITE_URL}/over/</loc>\n  </url>",
        f"  <url>\n    <loc>{SITE_URL}/edities/</loc>\n    <lastmod>{latest}</lastmod>\n  </url>"
    ]
